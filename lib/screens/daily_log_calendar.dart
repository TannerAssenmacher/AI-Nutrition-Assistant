...
class _DailyLogCalendarScreenState
  ...
  String _goalRangeLabel(_GoalCompletionRange range) {
    switch (range) {
  ...
  void _autoScrollToTodayIfNeeded(List<DateTime> weekDays) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!_weekListScrollController.hasClients) return;

      final todayContext = _todayCardKey.currentContext;
      if (todayContext == null) return;

      final renderObject = todayContext.findRenderObject();
      if (renderObject == null) return;

      final viewport = RenderAbstractViewport.of(renderObject);
      if (viewport == null) return;

      final position = _weekListScrollController.position;
      ...
      final visibleBottom = currentOffset + viewportExtent;
      final fullyVisible =
          revealTop >= visibleTop && revealBottom <= visibleBottom;
      ...
      if (revealTop < visibleTop) {
        targetOffset = revealTop;
      } else if (revealBottom > visibleBottom) {
        targetOffset = revealBottom - viewportExtent;
      }
  ...
  Map<String, double> _macroPercentagesFromProfile(AppUser? userProfile) {
    ...
    // If stored as grams, return default percentages
    return {'protein': 30.0, 'carbs': 40.0, 'fat': 30.0};
  }
  ...
  _ThirtyDayGoalStats _calculateGoalCompletionStats({
    required List<FoodItem>? log,
    required Map<String, double> goals,
    required _GoalCompletionRange range,
  }) {
    ...
    switch (range) {
      case _GoalCompletionRange.lastWeek:
        startDate = todayOnly.subtract(const Duration(days: 6));
        break;
      case _GoalCompletionRange.lastMonth:
        startDate = todayOnly.subtract(const Duration(days: 29));
        break;
      case _GoalCompletionRange.lastYear:
        startDate = todayOnly.subtract(const Duration(days: 364));
        break;
      case _GoalCompletionRange.lifetime:
        startDate = null;
        break;
    ...
    final daysToCheck = startDate == null
        ? allDays
        : allDays.where((day) => !day.isBefore(startDate!)).toList();
                    ...
                    children: [
                      ...
                      Expanded(
                        child: DropdownButtonFormField<String>(
                          ...
                          alignment: Alignment.center,
                          selectedItemBuilder: (context) => mealTypes
                              .map(
                                (type) => SizedBox(
                                  width: double.infinity,
                                  child: Text(
                                    _capitalizeMealType(type),
  ...
  Widget build(BuildContext context) {
    ...
    // Wait for both foodLog and userProfile to load
    return userProfileAsync.when(
      error: (e, _) => _wrapWithScaffold(
        SafeArea(child: Center(child: Text('Failed to load profile: $e'))),
      ),
      loading: () => _wrapWithScaffold(
        const SafeArea(child: Center(child: CircularProgressIndicator())),
      ),
      data: (userProfile) {
        final goals = _goalsFromProfile(userProfile);
        final macroPercentages = _macroPercentagesFromProfile(userProfile);
        ...
        final proteinPercent = macroPercentages['protein'] ?? 30.0;
        final carbsPercent = macroPercentages['carbs'] ?? 40.0;
        final fatPercent = macroPercentages['fat'] ?? 30.0;
        ...
        return foodLogAsync.when(
          ...
          data: (foodLog) {
            final goalCompletionStats = _calculateGoalCompletionStats(
              log: foodLog,
              goals: {
                'calories': calorieGoal,
                'protein': proteinGoal,
              ...
              SafeArea(
                ...
                child: Builder(
                  builder: (context) {
                    ...
                    final navVisualBottomSpacing =
                        mediaQuery.size.height * 0.008;
                    final navOverlayHeight =
                        (mediaQuery.size.height * 0.07) +
                        mediaQuery.viewPadding.bottom +
                        navVisualBottomSpacing;
                    final listBottomPadding = widget.isInPageView
                        ? 12.0
                        : (navOverlayHeight - 12.0)
                              .clamp(12.0, 300.0)
                              .toDouble();

                    return ListView(
                      controller: _weekListScrollController,
                      padding: EdgeInsets.fromLTRB(8, 0, 8, listBottomPadding),
                      children: [
                        Padding(
                          padding: EdgeInsets.only(
                            top: topInset,
                        ...
                        ...weekDays.map((day) {
                          ...
                          final loggedCount = foods.length;
                          final scheduledCount = scheduledMealsForDay.length;
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 10),
                              Text(
                                'Based on ${goalCompletionStats.consideredDays} day${goalCompletionStats.consideredDays == 1 ? '' : 's'}',
                                style: const TextStyle(
                                  fontSize: 12,
                                  fontWeight: FontWeight.w500,
                                  color: AppColors.textHint,
                                    ...
                                    children: [
                                      Expanded(
                                        child: _StatChip(
                                          label: 'Calories',
                                          pointsEarned:
                                              goalCompletionStats.caloriePoints,
                                          potentialPoints: goalCompletionStats
                                      ...
                                      Expanded(
                                        child: _StatChip(
                                          label: 'Protein',
                                          pointsEarned:
                                              goalCompletionStats.proteinPoints,
                                          potentialPoints: goalCompletionStats
                                              .potentialPointsPerCategory,
                                        ),
                                      ),
                                    ...
                                    children: [
                                      Expanded(
                                        child: _StatChip(
                                          label: 'Carbs',
                                          pointsEarned:
                                              goalCompletionStats.carbsPoints,
                                          potentialPoints: goalCompletionStats
                                              .potentialPointsPerCategory,
                                        ),
                                      ),
                              ...
                              const SizedBox(height: 10),
                              if (goalCompletionStats.perfectDays > 0)
                                Wrap(
                                  spacing: 4,
                                  runSpacing: 4,
                                  children: List.generate(
                                    goalCompletionStats.perfectDays,
                                    (_) => const Icon(
                                      Icons.star_rounded,
                                      size: 22,
                                      color: AppColors.statusNear,
...
class _EditableFoodCardState extends State<_EditableFoodCard> {
  ...
  void _showStatusSnack(String message, {bool isError = false}) {
    ...
    final dailyLogState = context
        .findAncestorStateOfType<_DailyLogCalendarScreenState>();
    final hasBottomNavBar =
        (dailyLogState?.widget.isInPageView ?? true) == false;
    final navVisualBottomSpacing = mediaQuery.size.height * 0.008;
    final navTopFromBottom =
        (mediaQuery.size.height * 0.07) + bottomInset + navVisualBottomSpacing;

    // Keep the status banner high enough to visually meet the nav bar's curved top.
    const estimatedBannerHeight = 38.0;
    final overlayBottom = hasBottomNavBar
        ? (navTopFromBottom - estimatedBannerHeight).clamp(bottomInset, 200.0)
        : bottomInset;
